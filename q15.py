...
class Grid:
    ...
    def move_unit(self, unit):
        ...
        if not enemy_locs:
            raise NoMoreEnemies()
        friend_locs = set(u.location for u in self.units if u.team ==
                          unit.team and u != unit and u.is_alive)
        ...
        while to_visit:
            ...
            for nb in pt.neighbours():
                ...
                if nb in visited:
                    ...
                    # looking futher
                    to_visit.append(
                        (nb, nb if next_loc is unit.location else next_loc))
        return

    def play_unit(self, unit):
        self.move_unit(unit)
        nbs = unit.location.neighbours()
        enemies = [u for u in self.units if u.team !=
                   unit.team and u.is_alive and u.location in nbs]
        if enemies:
